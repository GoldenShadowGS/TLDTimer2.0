...
void ClockFace::InitGeometry(ID2D1Factory2* pD2DFactory, float size)
{
	// Sun
	{
		...
		hr = sun.Geometry->Open(Sink.ReleaseAndGetAddressOf());
		if (FAILED(hr)) return;
	...
	// Moon
	{
		ComPtr<ID2D1GeometrySink> Sink;
		moon.size = size;
		HRESULT hr = pD2DFactory->CreatePathGeometry(moon.Geometry.ReleaseAndGetAddressOf());
		if (FAILED(hr)) return;

		hr = moon.Geometry->Open(&Sink);
		if (FAILED(hr)) return;
		Sink->SetFillMode(D2D1_FILL_MODE_WINDING);
		...
		Sink->AddArc(D2D1::ArcSegment(p2, D2D1::SizeF(size, size), 0.0f, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_LARGE));
		Sink->AddArc(D2D1::ArcSegment(p1, D2D1::SizeF(size * 0.7f, size * 0.35f), 0.0f, D2D1_SWEEP_DIRECTION_COUNTER_CLOCKWISE, D2D1_ARC_SIZE_LARGE));
...
void ClockFace::CreateGraphicsResources(ID2D1DeviceContext* pRenderTarget)
{
	...
	minutehandbitmap.Create(pRenderTarget, BITMAP_MINUTEHAND, RGB(220, 10, 220), pivot, scale);

	//offsetx = 0.088462f;
	//offsety = 0.48913f;
	pivot = D2D1::Point2F(0.088462f, 0.48913f);
	hourhandbitmap.Create(pRenderTarget, BITMAP_HOURHAND, RGB(140, 10, 140), pivot, scale);
...
void ClockFace::DrawBackGround(ID2D1DeviceContext* pRenderTarget, float angle)
{
	// Sun and moon
	{
		...
		D2D1_POINT_2F rotateoffset = rotate({ 0.0f, m_Radius * 0.5f }, angle);
		D2D1::Matrix3x2F translationmatrix = D2D1::Matrix3x2F::Translation(m_Center.x + rotateoffset.x, m_Center.y + rotateoffset.y);
		...
		rotateoffset = rotate({ 0.0f, m_Radius * 0.5f }, angle + PI);
		translationmatrix = D2D1::Matrix3x2F::Translation(m_Center.x + rotateoffset.x, m_Center.y + rotateoffset.y);
	...
	// Ticks
	{
		pRenderTarget->SetTransform(D2D1::Matrix3x2F::Translation(m_Center.x, m_Center.y));
		const float radius1 = m_Radius - 16.0f;
		const float radius2 = m_Radius - 8.0f;
		const float radiusOutside = m_Radius;
		for (int i = 0; i < 60; i++)
		{
...
{
	hourhandbitmap.Draw(pRenderTarget, hourAngle, m_Center);
	minutehandbitmap.Draw(pRenderTarget, minAgle, m_Center);
}
